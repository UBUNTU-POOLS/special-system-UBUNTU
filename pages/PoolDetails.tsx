...
import { FAMILY_WEALTH_TEMPLATE, SME_BULK_BUYING_TEMPLATE, SOUTH_AFRICAN_STOKVEL_TEMPLATE, EnhancedTemplate } from '../constitutionTemplate';
import { useCurrency } from '../contexts/CurrencyContext';
import { exportAuditPack, downloadJson } from '../reporting/exportBundle';
...
const PoolDetails: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'Ledger' | 'Members' | 'Governance' | 'Rules'>('Ledger');
  ...
  const filteredContributions = statusFilter === 'All' 
    ? MOCK_CONTRIBUTIONS 
    : MOCK_CONTRIBUTIONS.filter(c => c.status === statusFilter);

  const getTemplate = (): EnhancedTemplate => {
  ...
  const handleExport = async () => {
    setIsExporting(true);
    try {
      const pack = await exportAuditPack(pool.id, true);
      downloadJson(`ubuntu-pools-audit-pack-${pool.id}.json`, pack);
    } catch (err) {
  ...
  return (
    <div className="space-y-16 animate-in fade-in duration-1000 pb-40 relative">
      ...
      <section className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
        ...
        <div className="flex flex-wrap gap-4">
          <button 
            ...
            className="bg-white dark:bg-transparent border-2 border-[#2D302E] dark:border-[#FDFCFB] text-[#2D302E] dark:text-[#FDFCFB] px-8 py-6 rounded-[32px] text-xs font-black uppercase tracking-widest hover:bg-[#2D302E] hover:text-white dark:hover:bg-[#FDFCFB] dark:hover:text-[#2D302E] transition-all flex items-center gap-3 disabled:opacity-50"
          >
            {isExporting ? 'Generating Pack...' : 'Export Audit Pack'}
          </button>
      ...
      <div className="flex gap-12 border-b border-[#F1F0EE] dark:border-[#3A3D3B] transition-colors duration-500">
        {['Ledger', 'Members', 'Governance', 'Rules'].map((tab) => (
      ...
      {activeTab === 'Ledger' && (
        <div className="space-y-8 animate-in slide-in-from-bottom-4">
          <div className="flex gap-4 mb-2">
            {(['All', 'On Time', 'Late'] as const).map((filter) => (
              <button
                key={filter}
                onClick={() => setStatusFilter(filter)}
                className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${
                  statusFilter === filter 
                    ? 'bg-[#2D302E] dark:bg-[#FDFCFB] text-white dark:text-[#2D302E] shadow-lg' 
                    : 'bg-[#F9F9F8] dark:bg-[#1A1C1B] text-[#9EA39F] border border-[#F1F0EE] dark:border-[#3A3D3B]'
                }`}
              >
                {filter}
              </button>
          ...
          <div className="bg-white dark:bg-[#252826] rounded-[48px] border border-[#F1F0EE] dark:border-[#3A3D3B] shadow-sm overflow-hidden">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-[#F1F0EE] dark:divide-[#3A3D3B]">
                {filteredContributions.length > 0 ? filteredContributions.map((contribution) => (
                  ...
                  <tr>
                    <td colSpan={4} className="px-10 py-20 text-center text-[10px] font-black text-[#9EA39F] uppercase tracking-[0.4em]">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="border-b border-[#F1F0EE] dark:border-[#3A3D3B]">
                  <tr>
                    <th className="py-4 text-[10px] font-black uppercase tracking-widest text-[#9EA39F]">Accord Clause</th>
                    <th className="py-4 text-[10px] font-black uppercase tracking-widest text-[#9EA39F]">Code Module</th>
                    <th className="py-4 text-[10px] font-black uppercase tracking-widest text-[#9EA39F]">Permission Boundary</th>
                    <th className="py-4 text-[10px] font-black uppercase tracking-widest text-[#9EA39F]">Auth Record</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#F1F0EE] dark:divide-[#3A3D3B]">
                  {getTemplate().clauses.map((clause, i) => (
                    <tr key={i} className="group hover:bg-[#F9F9F8] dark:hover:bg-[#1A1C1B] transition-colors">
                      <td className="py-6 pr-8">
                        <p className="text-sm font-black text-[#2D302E] dark:text-[#FDFCFB] mb-1">{clause.title}</p>
                        <p className="text-[10px] font-medium text-[#6B706C] dark:text-[#9EA39F] line-clamp-1 italic">{clause.content}</p>
                      </td>
                      <td className="py-6 text-[10px] font-bold text-[#8CA082] tracking-wider uppercase pr-8">{clause.mapping.module}</td>
                      <td className="py-6 text-[10px] font-medium text-[#6B706C] dark:text-[#9EA39F] pr-8">{clause.mapping.permissionBoundary}</td>
                      <td className="py-6"><span className="text-[8px] font-black bg-[#8CA082]/10 text-[#8CA082] px-3 py-1 rounded-full uppercase tracking-widest">{clause.mapping.eventType}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="max-w-3xl space-y-12 animate-in slide-in-from-bottom-4">
          <div className="bg-[#E6D5C3]/20 dark:bg-[#3A3D3B]/20 p-12 rounded-[60px] space-y-8">
            ...
            <div className="space-y-8 text-sm font-medium text-[#6B706C] dark:text-[#9EA39F] leading-relaxed">
              {getTemplate().clauses.map(clause => (
